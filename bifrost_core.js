...
class BifrostCore {
    ...
    _parseZUIConfig(options, zuiConfigEarly, reconnectDelay, timeout) {
      ...
      if (zBlock && !autoRequest) {
        ...
        // SSOT gap closer: a dynamic route (e.g. /s/%slug) seats %route.* for
        // the server's OWN SSR gate/render, but that render ships an EMPTY
        // shell — THIS execute_walker is what actually paints the page's
        // %data.*-driven content, over a fresh WS round-trip that otherwise
        // has no idea a dynamic segment ever existed. routeParams rides in
        // zui-config next to zBlock/zVaFile/zVaFolder for exactly this reason
        // — hand it straight back so the server can re-seat %route.* before
        // resolving the SAME zMeta.zSpool the gate already read.
        if (zuiConfig.routeParams) {
          autoRequest.params = zuiConfig.routeParams;
        }
        // Store for logging after logger init
